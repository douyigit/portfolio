...
// Use regex or parsing to update sibergiris1
eval(dataFile.replace('const appData', 'global.appData'));

const siberQuestions = global.appData.sibergiris1;

let updatedQuestions = siberQuestions.map(q => {
    const id = q.id.toString();
    if (answersMap[id]) {
        q.answer = answersMap[id];
    }
    return q;
});
...
console.log('Successfully updated data.js with correct answers.');
